...
ans<-c()
p<--1
dir<- list.dirs(path = ".images_data/training",full.names = T)
...
# plotting an image
a<- final_matrix[50,1:100,1:100,1:3]
r <- a[,,1]
g <- a[,,2]
b <- a[,,3]
img <- rgbImage(r, g, b)
...
p<--1
testing_images=0
dir<- list.dirs(path = "./images_data/testing",full.names = T)
dir<- dir[1:no_classes+1]
...
if(!data_augmentation) {  
  ...
  model %>% fit_generator(
    flow_images_from_data(final_matrix, ans,gen_images,
                          batch_size=32),
    steps_per_epoch=as.integer(training_images/32),epochs = 50,
